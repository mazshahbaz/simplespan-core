...
import os, re, sys, json, argparse, datetime, hashlib
    ...
    for bucket, globs in paths.items():
        for g in globs or []:
            if Path(p).match(g):
                return bucket
    # heuristics
    if "/ai/" in p or "ai-response-protocol" in p: return "ai"
    if "/1-governance/" in p or "change-management-policy" in p: return "governance"
    if "/6-fea/" in p: return "fea"
    if "/5-modules/" in p: return "modules"
    if "/3-libraries/" in p: return "libraries"
    return "default"
...
def iter_docs(root: Path, only_glob=None, since=None):
    for p in root.rglob("*.md"):
        if ".venv" in p.parts or "node_modules" in p.parts:
            continue
        if only_glob and not Path(p.as_posix()).match(only_glob):
            continue
        if since:
...
def main():
    ...
    ap.add_argument("--report", default="docs/docs_audit_report.json", help="Path to write JSON report")
    ...
    ap.add_argument("--check-anchors", action="store_true", help="(Light) check anchors (best-effort)")
    ap.add_argument("--check-ai-logs", action="store_true", help="Verify ai_generated files are logged")
    args = ap.parse_args()
    ...
    for p in iter_docs(docs_root, only_glob=args.only, since=since_date):
        ...
        if "_yaml_error" in meta:
            (errors if args.strict else warnings).append(f"{p.as_posix()}: YAML error: {meta['_yaml_error']}")

        for k in REQUIRED_KEYS:
            if k not in meta:
                (errors if args.strict else warnings).append(f"{p.as_posix()}: missing front-matter key '{k}'")
        ...
        if lr:
            ...
            for fmt in ("%Y-%m-%d", "%Y/%m/%d", "%Y-%m-%dT%H:%M:%SZ"):
                try:
                    ...
                    continue
            if not d:
                (errors if args.strict else warnings).append(f"{p.as_posix()}: invalid last_reviewed '{lr}'")
        else:
        ...
        if d:
            ...
            if age > max_days:
                stale.append({"path": p.as_posix(), "last_reviewed": str(d), "age_days": age, "max_days": max_days})
                if args.fail_on_stale:
                    errors.append(f"{p.as_posix()}: stale by {age - max_days} days (bucket={bucket})")
                else:
                    warnings.append(f"{p.as_posix()}: stale by {age - max_days} days (bucket={bucket})")

        # AI log compliance
        if args.check_ai_logs and meta.get("ai_generated") is True:
            if not file_in_ai_logs(p, logs):
                (errors if args.strict else warnings).append(f"{p.as_posix()}: ai_generated true but no ai-log entry found")
        ...
        if args.check_links:
            for link in links:
                ...
                if not target_path.exists():
                    (errors if args.strict else warnings).append(f"{p.as_posix()}: broken link -> {link}")
        ...
        contexts.append({
            ...
            "links": links[:100],
            "summary_hash": hashlib.sha256((meta.get("title","") + "\n" + "\n".join(headings)).encode("utf-8")).hexdigest()[:16]
        })
    ...
    if not args.no_index:
        ...
        idx_path.write_text(json.dumps({"generated": datetime.datetime.utcnow().isoformat()+"Z",
                                        "files": contexts}, indent=2), encoding="utf-8")

    # Report
    status = "ok"
    if errors: status = "error"
    elif stale or warnings: status = "warn"

    report = {
        "status": status,
        "summary": {"files": files_scanned, "errors": len(errors), "warnings": len(warnings), "stale": len(stale)},
        "errors": errors,
        "warnings": warnings,
        "stale": stale
    }
    ...
    # Console summary
    print(f"[docs_audit] status={status} files={files_scanned} errors={len(errors)} warnings={len(warnings)} stale={len(stale)}")
    if errors:
        for e in errors:
